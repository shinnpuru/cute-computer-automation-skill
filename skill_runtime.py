...
def get_a11y_tree(max_depth: int | None = None) -> str:
    ...
    if device:
        ...
        header = f"A11Y_TREE source=android serial={serial} nodes={node_count}"
        return "\n".join([header, *output])
    ...
    if sys.platform == "win32":
        ...
        header = f"A11Y_TREE source=local platform=win32 nodes={node_count}"
        return "\n".join([header, *output])
    ...
    if sys.platform == "darwin":
        ...
        header = f"A11Y_TREE source=local platform=darwin nodes={node_count}"
        return "\n".join([header, *output])
    ...
    if sys.platform == "linux":
        ...
        def walk(node, depth: int) -> None:
            try:
                _append(depth, node.roleName, node.name, bounds=f"{node.position}, {node.size}")
                if max_depth is not None and depth >= max_depth:
                    return
        ...
        header = f"A11Y_TREE source=local platform=linux nodes={node_count}"
        return "\n".join([header, *output])
